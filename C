...
from azure.ai.projects.models import ChatMessage
...
def manage_blocklist(blocklist_name, block_items):
    ...
    # Add block items
    options = AddBlockItemsOptions(block_items=block_items)
    content_safety_client.add_block_items(blocklist_name, options)
...
def call_ai_foundry_model(prompt):
    deployment_name = os.environ["AZURE_AI_FOUNDRY_DEPLOYMENT_NAME"]
    response = inference_client.chat_completions.create(
...
def main():
    st.title("Azure AI Foundry & Content Safety Streamlit App")
    ...
    with tracer.start_as_current_span("user_interaction"):
        ...
        if st.button("Create/Update Blocklist"):
            with tracer.start_as_current_span("manage_blocklist"):
                block_items = [
                    TextBlockItemInfo(text=item.strip(), description=f"Blocked term: {item.strip()}")
        ...
        st.header("5. AI Foundry Model Invocation")
        prompt = st.text_area("Enter prompt for AI Foundry model:")
        if st.button("Get AI Response"):
            with tracer.start_as_current_span("ai_foundry_response"):
                response = call_ai_foundry_model(prompt)
                st.subheader("AI Foundry Response")
                st.write(response)
